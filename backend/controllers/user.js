...
exports.login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email })
      .select("+password")
      .populate("posts followers following requests savedPost");
...
exports.followUser = async (req, res) => {
  try {
    ...
    if (userFollow.requests.includes(loggedInUser._id)) {
      const indexOfRequest = userFollow.requests.indexOf(loggedInUser._id);
      userFollow.requests.splice(indexOfRequest, 1);
      await userFollow.save();
      return res.status(200).json({ success: true, message: "UnFollowed" });
    }

    userFollow.requests.unshift(loggedInUser._id);
...
exports.userRequest = async (req, res) => {
  try {
    ...
    loggedInUser.followers.push(userFollow._id);
    userFollow.following.push(loggedInUser._id);

    const indexOfRequest = loggedInUser.requests.indexOf(userFollow._id);
    loggedInUser.requests.splice(indexOfRequest, 1);
...
exports.rejectRequest = async (req, res) => {
  try {
    ...
    // const indexOfFollowing = userFollow.following.indexOf(userFollow._id)
    // userFollow.following.splice(indexOfFollowing, 1)
...
exports.myProfile = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate(
      "posts followers following requests story savedPost"
    );
...
exports.getUserProfile = async (req, res) => {
  try {
    const user = await User.findById(req.params.id).populate(
      "posts followers following requests"
    );
    if (!user) {
      return res
        .status(404)
